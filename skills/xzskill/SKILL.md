...
用户传入一个 skill 名称后，基于 `工程根目录/skills/<skill-name>/SKILL.md` 生成对应的手动安装文件，并同步更新 `README.md` 中的技能列表。
...
2. 读取 `工程根目录/skills/<skill-name>/SKILL.md`，将其作为唯一事实源。
3. 参考仓库现有格式，生成或更新：
...
4. 生成后同步更新 `README.md`：
   - 确保“当前已提供”列表包含 `工程根目录/skills/<skill-name>/SKILL.md`
...
`错误：工程根目录/skills/<skill-name>/SKILL.md 不存在，无法生成手动安装版本。`
